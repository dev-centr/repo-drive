/*
 * FUSE3 high-level shim for RepoDrive (Linux libfuse3 / macFUSE fuse3).
 * Calls D exports: rd_getattr, rd_readdir, rd_read.
 */
...
static int fill_cb(void *buf, const char *name, int is_dir) {
    ...
    return ctx->filler(ctx->buf, name, &st, 0, FUSE_FILL_DIR_PLUS);
}

static int rd_fuse_getattr(const char *path, struct stat *stbuf, struct fuse_file_info *fi) {
    (void)fi;
